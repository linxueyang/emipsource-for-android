Ext.define('YZSoft.src.panel.Social', {
    ...
    constructor: function (config) {
        ...
        me.store.on({
            load: function (s, records, successful, opts) {
                if (!successful)
                    return;
        ...
        me.list = Ext.create('Ext.dataview.List', {
            ...
            scrollToTopOnRefresh: false,
            //emptyText: RS.$('YZMobile.Communication_EmptyText'),
            pressedCls: '',
            itemTpl: new Ext.XTemplate(
                ...
                '<tpl else>',
                    '<div class="yz-layout-columns yz-layout-columns-other yz-layout-columns-{message:this.getMessageType}">',
                        ...
                        '</div>',
                        '<div class="yz-column-center">',
                            '<div class="time">{id},{date:this.renderDate}</div>',
                            '<div class="message-wrap">',
                                '<div class="yz-align-vcenter message-body">',
                                    '<div class="message">',
                                        '<div class="message-inner">{message:this.renderMessage}</div>',
    ...
    send: function () {
        ...
        var item = {};
        item.item = me.list.getItemAt(me.store.indexOf(r));
        item.statusEl = item.item.element.down('.yz-socialmessage-status');
    ...
    sendAudio: function (uri, duration) {
        ...
        YZSoft.src.device.File.upload({
            ...
            failure: function (action) {
                item.statusEl.removeCls('yz-socialmessage-send-waiting');
                Ext.Msg.alert('上传失败', action.result.errorMessage);
            },
    ...
    sendPicture: function (uri) {
        ...
        var item = {};
        item.item = me.list.getItemAt(me.store.indexOf(r));
        item.msgEl = item.item.element.down('.message-inner');
