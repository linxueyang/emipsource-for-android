...
Ext.define('YZSoft.src.viewmodel.express.Parser', {
    ItemType_None: 0,
    ItemType_Value: 1,
    ItemType_Operator: 2,
    ItemType_Function: 3,
    ...
    statics: {
        ...
        GreaterThanOp: function (a, b) {
            return a > b;
        },

        LesserThanOp: function (a, b) {
            return a < b;
        },

        GreaterEquOp: function (a, b) {
            return a >= b;
        },

        LesserEquOp: function (a, b) {
            return a <= b;
        },

        NotEquOp: function (a, b) {
            return a != b;
        },
        ...
        javascriptFn: function (fnname, a, b) {
            var me = this,
                args = Ext.toArray(arguments, 1),
                fn = me.form.funcs[fnname];

            if (!fn)
                YZSoft.Error.raise(RS.$('YZStrings.Form_Express_FuncNoDefine'), fnname);

            return fn.apply(me.form, args);
        },
        ...
        sumFn: function (vs) {
            ...
            for (var i = 0; i < vs.length; i++)
                rv = this.self.AddOp(rv, Number(vs[i]));
        ...
        avgFn: function (vs) {
            ...
            return vs.length ? this.self.DivOp(rv, vs.length) : 0;
        }
    },

    constructor: function (express,form) {
        var me = this;
        ...
        me.ops = [
            { fn: me.self.AddOp, pri: me.PRI_ADD, symbol: '+', unary: false },
            { fn: me.self.SubOp, pri: me.PRI_ADD, symbol: '-', unary: false },
            { fn: me.self.MultOp, pri: me.PRI_MULT, symbol: '*', unary: false },
            { fn: me.self.DivOp, pri: me.PRI_MULT, symbol: '/', unary: false },
            { fn: Math.pow, pri: me.PRI_EXP, symbol: '^', unary: false },
            { fn: me.self.MinusOp, pri: me.PRI_UNARY, symbol: '-', unary: true },
            { fn: me.self.ModOp, pri: me.PRI_MULT, symbol: '%', unary: false },
            { fn: me.self.GreaterEquOp, pri: me.PRI_BELOWADD, symbol: '>=', unary: false },
            { fn: me.self.LesserEquOp, pri: me.PRI_BELOWADD, symbol: '<=', unary: false },
            { fn: me.self.GreaterThanOp, pri: me.PRI_BELOWADD, symbol: '>', unary: false },
            { fn: me.self.LesserThanOp, pri: me.PRI_BELOWADD, symbol: '<', unary: false },
            { fn: me.self.NotEquOp, pri: me.PRI_BELOWADD, symbol: '!=', unary: false },
            { fn: me.self.AndOp, pri: me.PRI_LOWEST, symbol: '&&', unary: false },
            { fn: me.self.OrOp, pri: me.PRI_LOWEST, symbol: '||', unary: false },
            { fn: me.self.NotOp, pri: me.PRI_MULT, symbol: '!', unary: true },
            { fn: me.self.EquOp, pri: me.PRI_BELOWADD, symbol: '==', unary: false }
        ];
        ...
        me.functions = {
            ...
            "if": { fn: me.self.ifFn, nbArgs: 3, symbol: 'if', rpt: false },
            "javascript": { fn: me.self.javascriptFn, nbArgs: -1, symbol: 'javascript', rpt: false, fireEle: true },
            "min": { fn: me.self.minFn, nbArgs: 1, symbol: 'min', rpt: true },
            "max": { fn: me.self.maxFn, nbArgs: 1, symbol: 'max', rpt: true },
            "sum": { fn: me.self.sumFn, nbArgs: 1, symbol: 'sum', rpt: true },
            "avg": { fn: me.self.avgFn, nbArgs: 1, symbol: 'avg', rpt: true }
        };
    ...
    onOp: function (pos, wd, op) {
        ...
        this.setCurItemType(this.ItemType_Operator);

        var pitp = this.getPrevItemType();
        var unary = (pitp == this.ItemType_None || pitp == this.ItemType_Operator);
        ...
        if (op.unary != unary) {
            ...
            if (op.unary != unary)
                YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr001'), this.exp, op.symbol);
        }
        ...
        if (ops.length > 0) {
            ...
            var cpri;
            if (ppri != this.PRI_OPENBRACKET)
                cpri = op.pri;

            if (cpri == ppri && ops[ops.length - 1].itemType == this.ItemType_Operator && ops[ops.length - 1].eval.unary)
        ...
        var opi = {
            ...
            endWithABlock: false,
            eval: op,
            itemType: this.ItemType_Operator
        };
        ops.push(opi);
    },
    ...
    onOpenBracket: function (pos, wd) {
        ...
        var opi = {
            pri: this.PRI_OPENBRACKET,
            endWithABlock: false,
            itemType: this.ItemType_None
        };
        this.opStack.push(opi);

        if (this.getCurItemType() == this.ItemType_Function)
    ...
    onCloseArgSeparator: function (pos, wd, close) {
        ...
        if (this.getCurItemType() == this.ItemType_Operator)
            YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr002'), this.exp);
        ...
        if (!close) { //arg
            this.pushCurItemType();
            if (nc == ')')
                YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr003'), this.exp);
        }
        ...
        do {
            if (ops.length == 0) {
                if (close)
                    YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr004'), this.exp);
                else
                    YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr003'), this.exp);
            }

            cpri = ops[ops.length - 1].pri;
            if (cpri != this.PRI_OPENBRACKET) {
                popeditems++;
                var opi = ops.pop();
                this.addAndValidateToExprStack(opi);
            }
        } while (cpri != this.PRI_OPENBRACKET);

        var isInc = this.incArgCountIfArgValue(ops);
        if (close) {
            ops.pop();
            if (this.getCurItemType() != this.ItemType_Function)
                this.setCurItemType(this.ItemType_Value);
        }
    ...
    onExpEnd: function (pos, wd) {
        ...
        for (var i = ops.length - 1; i >= 0; i--) {
            var opi = ops[i];
            if (opi.pri == this.PRI_OPENBRACKET)
                YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr005'), this.exp);
        ...
        for (var i = 0; i < l; i++) {
            ...
            if (itm.eval && itm.eval.rpt) {
                ...
                for (j = i - 1; j >= 0; j--) {
                    ...
                    var args = itm1.args;
                    if (itm1.eval.rpt)
                        YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr006'), this.exp, itm1.eval.symbol);
    ...
    setArgValueFlag: function (b) {
        if (this.opStack.length > 0) {
            ...
            if (this.opStack[t].pri == this.PRI_OPENBRACKET)
                t--;
    ...
    pushCurItemType: function () {
        this.curItemTypeStack.push({ current: this.ItemType_None, previous: this.ItemType_None });
    },
    ...
    addAndValidateToExprStack: function (opi) {
        if (opi.itemType == this.ItemType_Function) {
            if (opi.eval.nbArgs != -1 && opi.eval.nbArgs != opi.nbArgs)
                YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr007'), this.exp, opi.eval.symbol, opi.eval.nbArgs);
        }
        ...
        catch (e) { YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr008'), e.message); }
    },
    ...
    parseWord: function (p, w) {
        ...
        if (citp != this.ItemType_Operator && citp != this.ItemType_None)
            YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr009'), this.exp);

        if (this.exp.charAt(p) == '(') {
            var f = this.functions[w];
            if (f) {
                this.setArgValueFlag(true);
                this.setCurItemType(this.ItemType_Function);
                ...
                this.opStack.push({
                    ...
                    eval: f,
                    argValue: false,
                    itemType: this.ItemType_Function
                });
            }
            else
                YZSoft.Error.raise(RS.$('YZStrings.Form_Express_SynErr010'), this.exp, w);
        }
    ...
    incArgCountIfArgValue: function (ops) {
        if (ops.length > 0) {
            ...
            if (ops[idx].pri == this.PRI_OPENBRACKET)
                idx--;

            if (idx >= 0) {
                if (ops[idx].argValue) {
                    if (ops[idx].itemType == this.ItemType_Function)
                        ops[idx].nbArgs++;
                    return true;
                }
            }
